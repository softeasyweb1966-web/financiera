document.addEventListener('DOMContentLoaded', () => {
    ...
    function actualizar() {
        ...
        const saldo = importe(form.elements.valor) - (credito && inicial ? importe(valorInicial) : 0);
        const suma = [...cuotas.querySelectorAll('[name="cuota_valor"]')].reduce((n, i) => n + importe(i), 0);
        document.getElementById('resumen-credito').textContent = 'Saldo a crédito: ' + moneda(saldo) + ' · Total programado: ' + moneda(suma) + ' · Diferencia: ' + moneda(saldo - suma);
        return {credito, saldo, suma};
    }
    document.getElementById('agregar-cuota-credito').addEventListener('click', () => {
        const fila = document.createElement('div'); fila.className = 'row g-2 mb-2 cuota-credito';
        fila.innerHTML = '<div class="col-5"><input aria-label="Fecha de cuota" name="cuota_fecha" type="date" class="form-control"></div><div class="col-5"><input aria-label="Valor de cuota" name="cuota_valor" class="form-control js-money" data-money-scale="2"></div><div class="col-2"><button type="button" class="btn btn-outline-danger quitar-cuota" aria-label="Quitar cuota">×</button></div>';
        cuotas.appendChild(fila); window.FinancieraMoney.init(fila); actualizar();
    });
    cuotas.addEventListener('click', e => {
        if (e.target.classList.contains('quitar-cuota')) {e.target.closest('.cuota-credito').remove(); actualizar();}
    });
    ...
    form.addEventListener('submit', e => {
        ...
        if (credito && (saldo <= 0 || saldo !== suma || !cuotas.children.length)) {
            e.preventDefault(); alert('Revise las cuotas: deben sumar exactamente el saldo pendiente de la compra.');
        }
    });
    actualizar();
});
